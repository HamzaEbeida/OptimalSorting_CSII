...
int sort(string array[], int arraySize, int letterIndex, int first) {
  ...
  for(int i=0; i < arraySize; i++) {
    int wordLetterIndex = getWordsLetterIndex(array[i+first], letterIndex);
    int index = lettersIterate[wordLetterIndex] - 1;
    words[index] = array[i+first];
    lettersIterate[wordLetterIndex]--;
  }

  for(int i = 0; i < arraySize; i++) array[i + first] = words[i];

  for(int i = 0; i < 26; i++) {
    if(letterCounts[i] < letterCounts[i + 1] - 1) sort(array, letterCounts[i + 1] - letterCounts[i] - 1, letterIn\
dex + 1, letterCounts[i]);
  }
